...
export async function handler(event) {
  ...
  try {
    ...
    // Optionally persist to a table like `document_files` if it exists
    if (supabase && payload) {
      try {
        await supabase.from("webhook_logs").insert({ kind: "document-file", payload, created_at: new Date().toISOString() });
      } catch {}
    }
